...
const eventsData = [
  {
    ...
    image: event1,
    description:
      "Hands-on Summer of Product workshop with HelloPM, led by Ankit Shukla.",
    tag: "Workshop",
  ...
  },
  {
    ...
    description:
      "Real-world consulting cases to test strategy, analytics, and problem-solving.",
    tag: "Competition",
    gradient: "from-blue-500 to-cyan-500",
  },
  {
    title: "Case Study Session",
    image: "/onlineSession.jpg",
    description: "Online Session for campus placement interviews organised by The Consulting Club of IIM Bangalore",
    tag: "Online Session",
    ...
    <section id="event-section" className="py-24 px-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
        <div className="space-y-4">
          ...
          <p className="text-gray-400 text-xl max-w-md">
            From raw ideas to measurable impact. Join our flagship sessions.
          </p>
        </div>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {eventsData.map((event, idx) => (
          <MotionLink
            to={event.tag == "Online Session" ? 'https://youtu.be/M5Kc4TS_Lg0?si=IsXnCOviYeNB7SL-' : ''}
            key={idx}
            whileHover={{ y: -10 }}
            className="glass-card group flex h-full flex-col overflow-hidden rounded-[2.5rem]"
          >
            <div
              className={`relative bg-gradient-to-br p-5 sm:p-6 ${event.gradient}`}
            >
